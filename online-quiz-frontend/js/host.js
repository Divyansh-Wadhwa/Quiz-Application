...
document.addEventListener("DOMContentLoaded", function () {
    ...
    quizForm.onsubmit = async function (e) {
        ...
        try {
            // First, create the quiz metadata
            const quizMetadata = {
                quizId: quizId,
                quizName: quizName,
                creator: getUsernameFromToken(),
                createdAt: new Date().toISOString(),
                questionCount: questions.length,
                isActive: true
            };

            // Submit quiz metadata (if your backend supports it)
            try {
                await fetch(`${window.QUESTION_API_BASE}/quiz/create`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${localStorage.getItem('token')}`
                    },
                    body: JSON.stringify(quizMetadata)
                });
            } catch (metaError) {
                console.log('Quiz metadata endpoint not available, proceeding with questions only');
            }
